...
- `epoll_create()` makes a new epoll instance, get back a file descriptor.
- `epoll_ctl(ADD / MOD / DEL)` starts, modifies, or stops watching some other file descriptor.
- `epoll_wait()` blocks until one of the watched fds has activity.
...
**`struct eventpoll`** is one per epoll instance (one per `epoll_create()` call). Holds the wait queue, the RB tree of items it's currently watching, and, critically for us, `refs`, an hlist head linking every `epitem` that points *back at this instance from somewhere else*.

**`struct epitem`** is one per (epoll instance, watched fd) pair. Holds `epi->ep`, a pointer back to its owning `eventpoll`. If the watched fd is itself an epoll, this `epitem` is also linked into *that* epoll's `refs` hlist via `fllink`.
